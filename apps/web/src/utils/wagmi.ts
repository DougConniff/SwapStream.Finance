...
const CHAINS = [mainnet, bsc, bscTest, rinkeby, goerli]
...
const getNodeRealUrl = (networkName: string) => {
  ...
  switch (networkName) {
    ...
    if (process.env.NEXT_PUBLIC_NODE_REAL_API_SEPOLIA) {
        host = `eth-sepolia.nodereal.io/v1/${process.env.NEXT_PUBLIC_NODE_REAL_API_SEPOLIA}`
      }
      break
    case 'goerli':
...
export const coinbaseConnector = new CoinbaseWalletConnector({
  ...
  options: {
    appName: 'SwapStream',
    appLogoUrl: 'https://swapstream.finance/logo.png',
  },
})
